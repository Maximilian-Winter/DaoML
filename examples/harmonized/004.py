...
class ImprovedWuWeiOptimizer(torch.optim.Optimizer):
            ...
            for p in group['params']:
                ...
                if len(p.shape) > 1:  # For matrices, calculate per-output significance
                    ...
                    if use_momentum_guidance:
                        ...
                        # Update momentum
                        self.param_momentum[param_id] = 0.9 * momentum + 0.1 * proposed_update
    ...
    def update_success_rates(self, loss_improvement):
        ...
        improvement_factor = 1.0 + 0.1 * torch.sign(loss_improvement).item()
...
class TaijiBatchNorm(nn.Module):
    ...
    def forward(self, x):
        if self.training:
            ...
            # Apply Taiji principle: balance between normalized and original
            balance = torch.sigmoid(self.yin_yang_balance).view(1, -1)

            # Store balance in history
            self.balance_history[self.history_index] = balance.squeeze()
            self.history_index = (self.history_index + 1) % self.balance_history.size(0)

            # Calculate oscillation waveform based on feature variance
            # High variance features get more normalization (balance -> 1)
            # Low variance features maintain more original signal (balance -> 0)
            relative_var = batch_var / (self.running_var + self.eps)
            var_adjusted_balance = balance * torch.sigmoid(relative_var.view(1, -1))

            # Apply the balanced normalization
            x = var_adjusted_balance * x_normalized + (1 - var_adjusted_balance) * x
...
class BaguaActivations(nn.Module):
    ...
    def adaptive_forward(self, x, metrics=None):
        ...
        # Get accuracy trend and loss trend - use scalar values, not tensors
        acc_increasing = metrics.get('acc_increasing', False)
        loss_decreasing = metrics.get('loss_decreasing', False)
        gradient_norm = float(metrics.get('gradient_norm', 1.0))  # Ensure it's a float
...
class SimpleModel(nn.Module):
    ...
    def __init__(self, input_size=9, hidden_sizes=[64, 32, 16], output_size=1,
        ...
        if self.use_loshu and input_size == 9 and hidden_sizes[0] == 9:
            self.loshu_matrix = create_positional_matrix(input_size, hidden_sizes[0])
        else:
            self.loshu_matrix = None
    ...
    def forward(self, x, metrics=None):
        ...
        # Apply bagua activation or standard ReLU
        if self.use_bagua:
            x = self.activations.adaptive_forward(x, metrics)
        else:
            x = F.relu(x)
        ...
        for i, (layer, norm) in enumerate(zip(self.hidden_layers, self.hidden_norms)):
            ...
            # Apply activation
            if self.use_bagua:
                # Use different trigrams for different layers
                trigram_idx = (i + 1) % 8
                trigram_name = self.activations.idx_to_trigram[trigram_idx]
                x = self.activations.forward(x, trigram_name)
            else:
                x = F.relu(x)
...
class DaoMLTrainer:
    ...
    def train_epoch(self, train_loader, val_loader=None):
        ...
        for batch_idx, (inputs, targets) in enumerate(train_loader):
            ...
            # KEY FIX: Create a detached copy of the metrics for the forward pass
            # This prevents issues with the computational graph
            forward_metrics = None
            if self.current_batch_metrics:
                forward_metrics = {k: v for k, v in self.current_batch_metrics.items()}

            # Forward pass with detached metrics
            outputs = self.model(inputs, forward_metrics)
            criterion = nn.BCEWithLogitsLoss()
            loss = criterion(outputs, targets)

            # Backward pass
            loss.backward()  # Add retain_graph=True if needed, but should not be necessary
            ...
            batch_loss = loss.item()  # Use .item() to detach from graph
            self.current_batch_metrics = {
                'loss': batch_loss,
                'accuracy': batch_acc,
                'loss_decreasing': batch_idx > 0 and batch_loss < self.current_batch_metrics['loss'],
                'acc_increasing': batch_idx > 0 and batch_acc > self.current_batch_metrics['accuracy'],
                'gradient_norm': grad_norm,
                'activation_mean': outputs.detach().mean().item(),  # Detach before mean
                'activation_std': outputs.detach().std().item(),  # Detach before std
            ...
            if batch_idx % 10 == 0:
                ...
                dominant_element = max(self.wu_xing_system.state.items(), key=lambda x: x[1])[0]
...
def main():
    """Main function demonstrating DaoML framework"""
    ...
    # Create DaoML model
    print("\nCreating model with DaoML components...")
    model = SimpleModel(
        input_size=9,
    ...
    # Create DaoML trainer
    print("\nInitializing DaoML trainer...")
    ...
    for epoch in range(epochs):
        # Train one epoch
        train_loss, train_acc = trainer.train_epoch(train_loader, val_loader)
        ...
        if (epoch + 1) % 5 == 0 or epoch == 0:
            print(f"Epoch {epoch + 1}/{epochs}, "
                  f"Train Loss: {train_loss:.4f}, Val Loss: {trainer.metrics['val_loss'][-1]:.4f}, "
                  f"Train Acc: {train_acc:.4f}, Val Acc: {trainer.metrics['val_acc'][-1]:.4f}, "
                  f"LR: {state['learning_rate']:.6f}, "
                  f"Hexagram: {state['hexagram']['index']} ({state['hexagram']['name']}), "
                  f"Element: {state['dominant_element'].name}")
    ...
    # Print hexagram statistics
    print("\nHexagram transition statistics:")
    hex_history = trainer.hexagram_tracker.hexagram_transition_history
    print(f"Total hexagram transitions: {len(hex_history)}")

    # Count occurrences of each hexagram
    hexagram_counts = {}
    for from_hex, to_hex in hex_history:
        if to_hex not in hexagram_counts:
            hexagram_counts[to_hex] = 0
        hexagram_counts[to_hex] += 1

    # Print most frequent hexagrams
    print("\nMost frequent hexagrams:")
    sorted_hexagrams = sorted(hexagram_counts.items(), key=lambda x: x[1], reverse=True)
    for hex_idx, count in sorted_hexagrams[:5]:
        hex_name = trainer.hexagram_tracker.hexagrams.hexagram_names.get(hex_idx, f"Hexagram {hex_idx}")
        print(f"  - {hex_idx} ({hex_name}): {count}")

    # Print final Wu Xing element strengths
    print("\nFinal Wu Xing element strengths:")
    element_strengths = trainer.wu_xing_system.get_element_strengths()
